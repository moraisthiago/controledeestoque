...
const estoquesRepository = {
    ...
    ler: (id: number, callback: (estoque?: Estoque) => void) => {
        const sql = 'SELECT * FROM estoques WHERE id_estoque = ?'
        const params = [id]
        database.get(sql, params, (_err, row) => callback(row))
    },
    atualizar: (id: number, estoque: Estoque, callback: (notFound: boolean) => void) => {
        const sql = 'UPDATE estoques SET nome = ?, descricao = ? WHERE id_estoque = ?'
    ...
    apagar: (id: number, callback: (notFound: boolean) => void) => {
        const sql = 'DELETE FROM estoques WHERE id_estoque = ?'
        const params = [id]
        database.run(sql, params, function(_err) {
            callback(this.changes === 0)
        })
